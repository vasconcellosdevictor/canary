...
function Npc:parseBankMessages(message, npc, creature, npcHandler)
	local messagesTable = {
		...
		["change"] = "There are three different coin types in this World: \z
                        100 copper coins equal 1 silver coin, 100 silver coins equal 1 gold coin. \z
                        So if you'd like to change 100 copper into 1 silver, \z
                        simply say '{change copper}' and then '1 silver'",
		["bank"] = "We are The Bank. Our institution's roots are lost to time. Our agreements are enforced draconically.We provide currency exchange services, you can {change} money in any bank. You can also access your {bank account}",
		...
		["basic"] = "You can check the {balance} of your bank account, {deposit} money or {withdraw} it. \z
                        You can also {transfer} money to other characters, provided that they have a vocation",
		["job"] = "I work in this branch. I can {change money} for you and help you with your bank account",
		["bank account"] = {
			"Everyone can have one. The big advantage is that you can access your money in every branch of The Bank! ...",
...
function Npc:parseBank(message, npc, creature, npcHandler)
	...
	if MsgContains(message, "balance") then
			...
			npcHandler:say(
				string.format(
					"I think you must be one of the richest inhabitants in the world! \z
                           Your account balance is %d copper.",
					balance
				),
				npc,
				creature
		...
		elseif balance >= 1000000 then
			npcHandler:say(
				string.format(
					"Wow, you have reached the magic number of a million pieces!!! \z
                           Your account balance is %d copper!",
					balance
				),
				npc,
		...
		elseif balance >= 100000 then
			...
			npcHandler:say(string.format("Your account balance is %d copper.", balance), npc, creature)
			return true
		end
		-- Deposit
	elseif MsgFind(message, "deposit all") then
		count[playerId] = player:getMoney()
		npcHandler:say(string.format("Would you really like to deposit %d copper?", count[playerId]), npc, creature)
	...
	elseif MsgContains(message, "deposit") then
		...
		count[playerId] = player:getMoney()
		npcHandler:say("Please tell me how much copper it is you would like to deposit.", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 1 then
		...
		if isValidMoney(count[playerId]) then
			...
			npcHandler:say("You do not have enough copper.", npc, creature)
			npcHandler:setTopic(playerId, 0)
			return true
		end
	elseif npcHandler:getTopic(playerId) == 2 then
		if MsgContains(message, "yes") then
			if Player.depositMoney(player, count[playerId]) then
				npcHandler:say(
					string.format(
						"Alright, we have added the amount of %d copper to your {balance}. \z
	...
	elseif MsgContains(message, "withdraw") then
		if string.match(message, "%d+") then
			count[playerId] = getMoneyCount(message)
			if isValidMoney(count[playerId]) then
				npcHandler:say(string.format("Are you sure you wish to withdraw %d copper from your bank account?", count[playerId]), npc, creature)
				npcHandler:setTopic(playerId, 7)
			else
				npcHandler:say("There is not enough copper on your account.", npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
			return true
		else
			npcHandler:say("Please tell me how much copper you would like to withdraw.", npc, creature)
			...
			if player:getFreeCapacity() >= getMoneyWeight(count[playerId]) then
				if player:getFreeBackpackSlots() >= totalPiles then
					if not player:withdrawMoney(count[playerId]) then
						...
						npcHandler:say(string.format("Here you are, %i copper. Please let me know if there is something else I can do for you.", count[playerId]), npc, creature)
					end
				else
					npcHandler:say(
						string.format(
							"Hold on, you don't have enough room in your backpack to carry all these coins. \nI don't want you to drop them on the floor, perhaps come back when you have more space in your backpack!\nYou will receive %i gold stacks (%i coins), %i silver stacks (%i coins), and %i copper stacks (%i coins). Please ensure you have at least %i free slots in your backpack.\n",
							crystalPiles,
							crystalCoins,
							platinumPiles,
							platinumCoins,
		...
		if string.match(message, "%d+") then
			...
			if MsgContains(message, "to") then
				if player:getName():lower() == transfer[playerId]:lower() then
					npcHandler:say("Fill in this field with person who receives your copper!", npc, creature)
					npcHandler:setTopic(playerId, 0)
					return true
				end
				...
				if playerName then
					...
					npcHandler:say(string.format("So you would like to transfer %d copper to %s?", count[playerId], string.titleCase(transfer[playerId])), npc, creature)
			...
			if isValidMoney(count[playerId]) then
				npcHandler:say(string.format("Who would you like transfer %d copper to?", count[playerId]), npc, creature)
				npcHandler:setTopic(playerId, 12)
				return true
			end
		end
		npcHandler:say("Please tell me the amount of copper you would like to transfer.", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 12 then
		...
		if playerName then
			...
			npcHandler:say(string.format("So you would like to transfer %d copper to %s?", count[playerId], playerName), npc, creature)
			npcHandler:setTopic(playerId, 13)
		else
			npcHandler:say("This player does not exist.", npc, creature)
			npcHandler:setTopic(playerId, 0)
	...
	elseif npcHandler:getTopic(playerId) == 13 then
		if MsgContains(message, "yes") then
			if not player:transferMoneyTo(transfer[playerId], count[playerId]) then
				...
				npcHandler:say(string.format("Very well. You have transferred %d copper to %s.", count[playerId], string.titleCase(transfer[playerId])), npc, creature)
		...
		npcHandler:setTopic(playerId, 0)
		-- Change money
	elseif MsgContains(message, "change copper") then
		npcHandler:say("How many silver coins would you like to get?", npc, creature)
		npcHandler:setTopic(playerId, 14)
	elseif npcHandler:getTopic(playerId) == 14 then
		if getMoneyCount(message) < 1 then
			npcHandler:say("Sorry, you do not have enough copper coins.", npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			count[playerId] = getMoneyCount(message)
			npcHandler:say(string.format("So you would like me to change %d of your copper coins into %d silver coins?", count[playerId] * 100, count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 15)
		end
	elseif npcHandler:getTopic(playerId) == 15 then
		if MsgContains(message, "yes") then
			if player:removeItem(ITEM_COPPER_COIN, count[playerId] * 100) then
				player:addItem(ITEM_SILVER_COIN, count[playerId])
		...
		npcHandler:setTopic(playerId, 0)
	elseif MsgContains(message, "change silver") then
		npcHandler:say("Would you like to change your silver coins into {copper} or {gold}?", npc, creature)
		npcHandler:setTopic(playerId, 16)
	elseif npcHandler:getTopic(playerId) == 16 then
		if MsgContains(message, "copper") then
			npcHandler:say("How many silver coins would you like to change into {copper}?", npc, creature)
			npcHandler:setTopic(playerId, 17)
		elseif MsgContains(message, "gold") then
			npcHandler:say("How many gold coins would you like to get?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 17 then
		if getMoneyCount(message) < 1 then
			npcHandler:say("Sorry, you do not have enough silver coins.", npc, creature)
			npcHandler:setTopic(playerId, 0)
		else
			count[playerId] = getMoneyCount(message)
			npcHandler:say(string.format("So you would like me to change %d of your silver coins into %d copper coins for you?", count[playerId] * 100, count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 18)
		end
	elseif npcHandler:getTopic(playerId) == 18 then
		if MsgContains(message, "yes") then
			if player:removeItem(ITEM_SILVER_COIN, count[playerId]) then
				player:addItem(ITEM_COPPER_COIN, count[playerId] * 100)
				npcHandler:say("Here you are.", npc, creature)
			else
				npcHandler:say("Sorry, you do not have enough silver coins.", npc, creature)
			end
	...
	elseif npcHandler:getTopic(playerId) == 19 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say(string.format("So you would like me to change %d of your silver coins into %d gold coins for you?", count[playerId] * 100, count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 20)
		end
	elseif npcHandler:getTopic(playerId) == 20 then
		if MsgContains(message, "yes") then
			if player:removeItem(ITEM_SILVER_COIN, count[playerId] * 100) then
				player:addItem(ITEM_GOLD_COIN, count[playerId])
				npcHandler:say("Here you are.", npc, creature)
			else
				npcHandler:say("Sorry, you do not have enough silver coins.", npc, creature)
			end
	...
	elseif MsgContains(message, "change gold") then
		npcHandler:say("How many gold coins would you like to change into silver?", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 21 then
		if getMoneyCount(message) < 1 then
			...
			count[playerId] = getMoneyCount(message)
			npcHandler:say(string.format("So you would like me to change %d of your gold coins into %d silver coins for you?", count[playerId] * 100, count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 22)
		end
	elseif npcHandler:getTopic(playerId) == 22 then
		if MsgContains(message, "yes") then
			if player:removeItem(ITEM_GOLD_COIN, count[playerId]) then
				player:addItem(ITEM_SILVER_COIN, count[playerId] * 100)
				npcHandler:say("Here you are.", npc, creature)
			else
				npcHandler:say("Sorry, you do not have enough gold coins.", npc, creature)
			end
...
function Npc:parseGuildBank(message, npc, creature, playerId, npcHandler)
	...
	if MsgContains(message, "guild balance") then
		...
		npcHandler:say(string.format("Your guild account balance is %d copper.", Bank.balance(player:getGuild())), npc, creature)
		return true
		-- Guild deposit
	elseif MsgFind(message, "guild deposit") then
		...
		if string.match(message, "%d+") then
			...
			npcHandler:say(string.format("Would you really like to deposit %d copper to your {guild account}?", count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 123)
			return true
		else
			npcHandler:say("Please tell me how much copper it is you would like to deposit into your guild account.", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 123 then
		if MsgContains(message, "yes") then
			npcHandler:say(
				string.format(
					"Alright, we have placed an order to deposit the amount of %d copper to \z
	...
	elseif MsgContains(message, "guild withdraw") then
		...
		if string.match(message, "%d+") then
			count[playerId] = getMoneyCount(message)
			if isValidMoney(count[playerId]) then
				npcHandler:say("Are you sure you wish to withdraw %d copper from your guild account?", npc, creature)
				npcHandler:setTopic(playerId, 125)
			else
				npcHandler:say("There is not enough copper on your guild account.", npc, creature)
				npcHandler:setTopic(playerId, 0)
			end
			return true
		else
			npcHandler:say("Please tell me how much copper you would like to withdraw from your guild account.", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 124 then
		count[playerId] = getMoneyCount(message)
		if isValidMoney(count[playerId]) then
			npcHandler:say(string.format("Are you sure you wish to withdraw %d copper from your guild account?", count[playerId]), npc, creature)
			npcHandler:setTopic(playerId, 125)
		else
			npcHandler:say("There is not enough copper on your guild account.", npc, creature)
			npcHandler:setTopic(playerId, 0)
	...
	elseif npcHandler:getTopic(playerId) == 125 then
		if MsgContains(message, "yes") then
			...
			npcHandler:say(
				string.format(
					"We placed an order to withdraw %d copper from your guild account. \z
		...
		if string.match(message, "%d+") then
			...
			if isValidMoney(count[playerId]) then
				...
				if Game.getNormalizedGuildName(transfer[playerId]) then
					npcHandler:say(string.format("So you would like to transfer %d copper from your guild account to guild %s?", count[playerId], guildName), npc, creature)
					npcHandler:setTopic(playerId, 128)
				else
					npcHandler:say(string.format("Which guild would you like to transfer %d copper to?", count[playerId]), npc, creature)
			...
			npcHandler:say("Please tell me the amount of copper you would like to transfer to a guild.", npc, creature)
	...
	elseif npcHandler:getTopic(playerId) == 128 then
		if MsgContains(message, "yes") then
			npcHandler:say(string.format("We have placed an order to transfer %d copper from your guild account to guild %s.  Please check your inbox for confirmation.", count[playerId], string.titleCase(transfer[playerId])), npc, creature)
			local guild = player:getGuild()
			local info = {
			...
			if Bank.transferToGuild(guild, transfer[playerId], amount) then
				logger.info("Guild {} transferred {} copper to guild {}.", guild:getName(), amount, transfer[playerId])
				info.success = true
				info.message = "We are happy to inform you that your transfer request was successfully carried out."
			else
				logger.info("Guild {} failed to transfer {} copper to guild {}.", guild:getName(), amount, transfer[playerId])
