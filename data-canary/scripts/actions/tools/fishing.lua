local waterIds = { 4597, 4598, 4599, 4600, 4601, 4602, 4609, 4610, 4611, 4612, 4613, 4614, 21302, 21303, 21304, 21305, 21306, 21307, 21311, 21312, 21313, 21314, 21315 }
...
function fishing.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if math.random(1, 100) <= math.min(math.max(10 + (player:getEffectiveSkillLevel(SKILL_FISHING) - 10) * 0.597, 10), 50) then
		...
		if target.itemid == 21314 then
			target:transform(target.itemid + 1)
			target:decay()

			if math.random(1, 100) >= 97 then
				player:addItem(49282, 1)
				return true
			end
		elseif target.itemid == 21312 then
			target:transform(target.itemid + 1)
			target:decay()

			local rareChance = math.random(1, 100)
			if rareChance == 1 then
				player:addItem(49283, 1)
				return true
			elseif rareChance <= 4 then
				player:addItem(49283, 1)
				return true
